  ...
  <main class="flex min-h-screen items-center justify-center bg-[var(--crm-bg-page)] px-6 py-10">
    <Card class="w-full max-w-md">
      <CardHeader>
        ...
        <CardDescription>重置凭证仅可使用一次，成功后旧会话将失效。</CardDescription>
      </CardHeader>
      <CardContent>
        ...
        <form v-else class="space-y-4" @submit.prevent="submitForm">
          <Alert v-if="!credential || errorMessage" variant="destructive">
            <AlertDescription>
              {{ errorMessage || '重置链接缺少有效凭证，请重新发起找回。' }}
            </AlertDescription>
          </Alert>
          <div class="space-y-2">
...
import { resetPassword } from '@/modules/user/api/credential-api'
import {
...
const { defineField, errors, handleSubmit, resetForm } = useForm<ResetPasswordFormValues>({
...
const submitForm = handleSubmit(async (values) => {
  ...
  errorMessage.value = ''
  try {
    await resetPassword({ credential: credential.value, newPassword: values.newPassword })
    resetForm({ values: { newPassword: '', confirmPassword: '' } })
    credential.value = ''
    completed.value = true
  } catch (error: unknown) {
    resetForm({ values: { newPassword: '', confirmPassword: '' } })
    errorMessage.value = getCredentialCommandErrorMessage(error, 'reset')
