...
public class TokenVerifyFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        if (!gate.allowed()) {
            ...
            ResponseUtils.write(response, JSONUtils.toJSON(R.FAIL(gate.code().getCode(), gate.message())));
            return;
        }
        ...
        if (Boolean.TRUE.equals(currentUser.getMustChangePassword()) && !isFirstChangeAllowed(request)) {
            response.setStatus(HttpServletResponse.SC_FORBIDDEN);
            ResponseUtils.write(response, JSONUtils.toJSON(R.FAIL(CodeEnum.ACCESS_DENIED.getCode(), "首次改密完成前不能访问业务接口")));
            return;
        }
        filterChain.doFilter(request, response);
    }
