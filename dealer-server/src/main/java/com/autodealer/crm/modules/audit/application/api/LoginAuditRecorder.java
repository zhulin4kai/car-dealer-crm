...
public class LoginAuditRecorder {
    ...
    private final LoginAuditUserLookup userLookup;

    public LoginAuditRecorder(TLoginLogMapper loginLogMapper, LoginAuditUserLookup userLookup) {
    ...
    private void bindKnownUser(TLoginLog logEntry, String loginAct) {
        ...
        try {
            userLookup.findByLoginAct(loginAct).ifPresent(user -> {
                logEntry.setUserId(user.userId());
                logEntry.setUserName(limit(user.userName(), 64));
            });
