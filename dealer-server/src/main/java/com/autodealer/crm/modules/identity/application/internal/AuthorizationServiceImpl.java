...
public class AuthorizationServiceImpl implements AuthorizationService {
    ...
    private void validateCustomOrganizations(TPermission permission,TUser target,DataScopeCode scope,List<Integer>organizationIds){if(scope!=DataScopeCode.CUSTOM_ORGS){if(!organizationIds.isEmpty())throw new BusinessException(CodeEnum.PARAM_ERROR,"非指定组织范围不得携带组织");return;}if(organizationIds.isEmpty())throw new BusinessException(CodeEnum.PARAM_ERROR,"指定组织范围不能为空");for(Integer id:organizationIds){TOrganizationUnit organization=organizationMapper.selectByPrimaryKey(id);if(organization==null||!Boolean.TRUE.equals(organization.getEnabled())||Boolean.TRUE.equals(organization.getMigrationPlaceholder()))throw new BusinessException(CodeEnum.PARAM_ERROR,"指定组织不存在或不可用");}Set<Integer>delegable=new HashSet<>(policy.delegableCustomOrganizationIds(permission,target));if(!delegable.containsAll(organizationIds))throw new BusinessException(CodeEnum.ACCESS_DENIED,"指定组织超过操作者可委派范围或不适用于目标用户");}
