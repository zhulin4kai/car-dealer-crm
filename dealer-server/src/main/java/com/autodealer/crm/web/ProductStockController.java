...
public class ProductStockController {
    ...
    @GetMapping("/vehicles")
    @PreAuthorize("hasAuthority('" + PermissionCodes.PRODUCT_STOCK_VIEW + "')")
    public R<PageInfo<TProductVehicle>> getVehicles(ProductVehicleQuery query) {
        return R.OK(productVehicleService.getVehiclePage(query));
    }

    @PostMapping("/vehicles")
    @PreAuthorize("hasAuthority('" + PermissionCodes.PRODUCT_STOCK_ADJUST + "')")
    public R<TProductVehicle> inboundVehicle(@Valid @RequestBody CreateProductVehicleRequest request) {
        return R.OK(productVehicleService.inboundVehicle(request));
    }

    @PostMapping("/vehicles/{vehicleId}/reserve")
    @PreAuthorize("hasAuthority('" + PermissionCodes.PRODUCT_STOCK_ADJUST + "')")
    public R<TProductVehicle> reserveVehicle(@PathVariable Long vehicleId,
                                             @Valid @RequestBody ReserveProductVehicleRequest request) {
        return R.OK(productVehicleService.reserveVehicle(vehicleId, request));
    }

    @PostMapping("/vehicles/{vehicleId}/release")
    @PreAuthorize("hasAuthority('" + PermissionCodes.PRODUCT_STOCK_ADJUST + "')")
    public R<TProductVehicle> releaseVehicle(@PathVariable Long vehicleId,
                                             @Valid @RequestBody ReleaseProductVehicleRequest request) {
        return R.OK(productVehicleService.releaseVehicle(vehicleId, request));
    }
}
