...
class RoleServiceImplTest {
 ...
 @Mock RedisManager redis; @Mock TAuthorizationGraphLockMapper graphLock; @Mock UserAuthorizationPolicy authorizationPolicy; @Mock OwnerCandidateCacheInvalidator ownerCandidateCacheInvalidator; RoleAccessServiceImpl service;
 @BeforeEach void setup(){lenient().when(graphLock.lockByName(anyString())).thenAnswer(invocation->invocation.getArgument(0));service=new RoleAccessServiceImpl(roles,permissions,matrices,roleOrgs,rolePermissionOrgs,orgs,userRoles,users,employees,assignments,reporting,audit,current,redis,new ObjectMapper(),graphLock,authorizationPolicy,ownerCandidateCacheInvalidator);}
