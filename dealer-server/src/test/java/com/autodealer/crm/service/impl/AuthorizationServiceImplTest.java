...
import com.autodealer.crm.dto.access.UserAuthorizationDtos.BatchTarget;
import com.autodealer.crm.dto.access.UserAuthorizationDtos.BatchUpdatePermissionsRequest;
import com.autodealer.crm.dto.access.UserAuthorizationDtos.BatchUpdateRolesRequest;
...
class AuthorizationServiceImplTest {
    ...
    void personalPermissionWriteLocksMembershipBeforeTargetAndDelegationChecks() {
        TUser target=user(2,0);when(userMapper.selectByPrimaryKey(2)).thenReturn(target);
        com.autodealer.crm.model.TPermission permission=new com.autodealer.crm.model.TPermission();
    ...
    void customOrganizationGrantRejectsEnabledOrganizationOutsideDelegableTargetSubset() {
        ...
        permission.setId(7);permission.setEnabled(1);
        when(permissionMapper.selectByPrimaryKey(7)).thenReturn(permission);
        com.autodealer.crm.model.TOrganizationUnit organization=new com.autodealer.crm.model.TOrganizationUnit();
    ...
    void futurePermissionPersistsRequestedScheduleInsteadOfForcingCurrentTime() {
        ...
        when(policy.canDelegatePermission(eq(permission),eq(com.autodealer.crm.enums.DataScopeCode.SELF),eq(List.of()),eq(target))).thenReturn(true);
        when(userMapper.incrementAuthorizationVersionsByExpected(2,0)).thenReturn(1);
    ...
    void inheritClosesFuturePermissionWithoutDeletingPlanOrOrganizationFacts() {
        ...
        before.setEffect(com.autodealer.crm.enums.PermissionEffect.GRANT);before.setDataScopeCode(com.autodealer.crm.enums.DataScopeCode.CUSTOM_ORGS);
        ...
        verify(auditRecorder).recordAll(argThat(histories->histories.size()==1
                ...
                eq(com.autodealer.crm.audit.AuditActionEnum.USER_PERMISSION_CHANGE),eq("2"),contains("\"count\":1"));
    }
    ...
    void batchPermissionGrantAppliesSameValidatedChangeToEveryTarget() {
        ...
        com.autodealer.crm.model.TPermission permission=new com.autodealer.crm.model.TPermission();
        permission.setId(7);permission.setEnabled(1);when(permissionMapper.selectByPrimaryKey(7)).thenReturn(permission);
        when(policy.canDelegatePermission(eq(permission),eq(com.autodealer.crm.enums.DataScopeCode.SELF),eq(List.of()),any())).thenReturn(true);
        when(userMapper.incrementAuthorizationVersionsByExpected(2,0)).thenReturn(1);
